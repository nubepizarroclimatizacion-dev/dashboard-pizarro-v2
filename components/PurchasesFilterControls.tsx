...
interface PurchasesFilterControlsProps {
  providers: string[];
  years: string[];
...
const PurchasesFilterControls: React.FC<PurchasesFilterControlsProps> = ({
  providers, years, months, filters, onFilterChange, onResetFilters,
}) => {
    ...
    <div className="bg-white p-4 rounded-lg shadow-md mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MultiSelectFilter
            label="Proveedor"
            options={providers}
            selectedOptions={filters.providers}
            onChange={(selected) => onFilterChange('providers', selected)}
        />
        <MultiSelectFilter
            label="Año"
            options={years}
        ...
        <MultiSelectFilter
            ...
            selectedOptions={filters.modalities}
            onChange={(selected) => onFilterChange('modalities', selected)}
        />
      </div>
    </div>
  );
...
export default React.memo(PurchasesFilterControls);
